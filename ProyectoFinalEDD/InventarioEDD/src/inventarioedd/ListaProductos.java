...
// =======================
    ...
    // =======================
    /**
     * Ordena los productos por ID utilizando el algoritmo de Inserción. Cumple
     * con el requisito de ordenamiento del proyecto.
     */
    public void ordenarPorIdInsercion() {

        if (cabeza == null || cabeza.siguiente == null) {
            return; // Lista vacía o con un solo elemento
        }

        NodoProducto ordenada = null;
        NodoProducto actual = cabeza;

        while (actual != null) {
            NodoProducto siguiente = actual.siguiente;

            if (ordenada == null || actual.dato.getId() < ordenada.dato.getId()) {
                actual.siguiente = ordenada;
                ordenada = actual;
            } else {
                NodoProducto temp = ordenada;

                while (temp.siguiente != null
                        && temp.siguiente.dato.getId() < actual.dato.getId()) {
                    temp = temp.siguiente;
                }
    ...
    /**
     * Permite modificar valores agregados al inventario buscandolos desde su ID
     */
    public boolean modificarProducto(int id, String nombre, String categoria, int cantidad, int stockMinimo) {
        NodoProducto actual = cabeza;

        while (actual != null) {
            if (actual.dato.getId() == id) {
                actual.dato.setNombre(nombre);
                actual.dato.setCategoria(categoria);
                actual.dato.setCantidad(cantidad);
                actual.dato.setStockMinimo(stockMinimo);
                return true;
            }
            actual = actual.siguiente;
        }
